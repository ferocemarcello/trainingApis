...
import cloudscraper
from idbutils import RestClient, RestResponseException
...
def login(self):
    ...
    user_prefs = __get_json(response.text, 'VIEWER_USERPREFERENCES')
    modern_rest_client.save_json_to_file('./profile.json', self.user_prefs)
    self.display_name = self.user_prefs['displayName']
    social_profile = __get_json(response.text, 'VIEWER_SOCIAL_PROFILE')
